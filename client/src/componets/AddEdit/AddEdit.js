...
function AddEdit({ id, setPopUp, popUp }) {
  ...
  const handleChange = (e) => {
    ...
    console.log(name);
    const player1 = player.playerStatus;
    console.log(player1);
    type === "checkbox"
      ? setPlayer({ ...player1, [name]: value })
      : setPlayer({ ...player, [name]: value });
  };
  console.log(player);
  const handleSubmit = async (e) => {
    e.preventDefault();
    setTest({ ...test, player, isChecked });
    console.log(test);
    // const arr = [];
    ...
    //   }
    // }
    // const playerStatus = JSON.stringify(arr);
    // const playerStatus1 = arr.toString();
    // console.log(playerStatus1);
    // console.log(playerStatus);
    if (!id) {
      //add new recode
      const response = await axios.post("http://localhost:5000/api/post", test);
  ...
  console.log(player.playerStatus[checkBox[0].playerStatus]);
  const handleRest = () => {
    //reset form
    setPlayer(defaultValue);
  };
  ...
  console.log(player.playerStatus[checkBox1[0].playerStatus]);
  useEffect(() => {
    if (id) {
      loadData();
    }
    ...
    <div>
      <form onSubmit={handleSubmit}>
        ...
        {/* <div className="radioDiv">
          <input
            type="checkbox"
            value={checkBox[0].playerStatus}
            name={checkBox1[0].playerStatus}
            onChange={handleChange}
            checked={player.playerStatus.}
          />
          <label>{checkBox[0].playerStatus}</label>){" "}
        </div> */}
